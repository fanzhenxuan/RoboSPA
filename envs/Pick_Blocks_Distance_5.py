...
class Pick_Blocks_Distance_5(Base_Task):
    """
    任务说明：
    - 桌面上有10个不同颜色的block
    - 不再按颜色直接pick，而是按“相对参照物的距离排序”进行pick
    - 每轮随机选择一个参照block，再在其余9个block中随机选择“第k远”的block作为目标
    - 距离排序按“从远到近”定义，k的范围为1st ~ 9th
    ...
    def load_actors(self):
        ...
        size = np.random.uniform(0.012, 0.014)
        half_size = (size, size, size)

        min_center_dist = 0.068
        clear_dist_gap = 0.05
        table_z = 0.745
        ...
        self.cyan_block = create_box(
            scene=self,
            pose=block_pose_lst[6],
            half_size=half_size,
            color=(0.0, 1.0, 1.0),
            name="cyan_block",
        )

        self.pink_block = create_box(
            scene=self,
            pose=block_pose_lst[7],
            half_size=half_size,
            color=(1.0, 0.4, 0.7),
            name="pink_block",
        )

        self.brown_block = create_box(
            scene=self,
            pose=block_pose_lst[8],
            half_size=half_size,
            color=(0.55, 0.27, 0.07),
            name="brown_block",
        )

        self.gray_block = create_box(
            scene=self,
            pose=block_pose_lst[9],
            half_size=half_size,
            color=(0.5, 0.5, 0.5),
            name="gray_block",
        )
        ...
        self.blocks = {
            ...
            "cyan": self.cyan_block,
            "pink": self.pink_block,
            "brown": self.brown_block,
            "gray": self.gray_block,
        }
