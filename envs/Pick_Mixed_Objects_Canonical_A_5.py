...
class Pick_Mixed_Objects_Canonical_A_5(Base_Task):

    OBJECT_SPECS = [{'modelname': '073_rubikscube', 'skip_model_ids': []}, 
                    {'modelname': '046_alarm-clock', 'skip_model_ids': [0,1,2,3]}, 
                    {'modelname': '047_mouse', 'skip_model_ids': []}, 
                    {'modelname': '048_stapler', 'skip_model_ids': []}, 
                    {'modelname': '050_bell', 'skip_model_ids': []}, 
                    {'modelname': '006_hamburg', 'skip_model_ids': []}, 
                    {'modelname': '057_toycar', 'skip_model_ids': []}, 
                    {'modelname': '081_playingcards', 'skip_model_ids': []}, 
                    {'modelname': '039_mug', 'skip_model_ids': [0,5,12]}, 
                    {'modelname': '077_phone', 'skip_model_ids': []}, 
                    {'modelname': '095_glue', 'skip_model_ids': []}, 
                    {'modelname': '100_seal', 'skip_model_ids': []}
                    ]
    ...
    def load_actors(self):
        ...
        center_x = np.random.uniform(-0.04, -0.02)
        ...
        poses = [
            fixed_item_pose(x1, y1), fixed_item_pose(x2, y1), fixed_item_pose(x3, y1), fixed_item_pose(x4, y1),
            fixed_item_pose(x1, y2), fixed_item_pose(x2, y2), fixed_item_pose(x3, y2), fixed_item_pose(x4, y2),
            fixed_item_pose(x1, y3), fixed_item_pose(x2, y3), fixed_item_pose(x3, y3), fixed_item_pose(x4, y3),
        ]
        ...
        self.items = [create_item(poses[i], self.object_specs[i]) for i in range(12)]
