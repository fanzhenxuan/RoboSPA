...
def parse_img_array(data):
    ...
    Args:
        data: np.ndarray of shape (N,), where each element is either Python bytes
            or an np.ndarray with dtype uint8.

    Returns:
        imgs: np.ndarray of shape (N, H, W, C), with dtype uint8.
    """
    flat = data.ravel()
    ...
    for buf in flat:
        ...
        if img is None:
            raise ValueError("cv2.imdecode returned None, indicating that the byte stream may not be in a valid image format.")
        imgs.append(img)
