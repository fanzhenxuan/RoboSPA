...
class Pick_Mixed_Objects_Canonical_A_4(Base_Task):
    ...
    def _sampled_specs(self):
        specs = []
        for spec in self.OBJECT_SPECS[:9]:
            spec = dict(spec)
    ...
    def load_actors(self):
        ...
        center_x = np.random.uniform(-0.08, 0.02)
        center_y = np.random.uniform(-0.11, -0.1)
        col_gap = 0.156
        row_gap = 0.125
        x1 = center_x - col_gap
        x2 = center_x
        ...
        self.items = [create_item(poses[i], self.object_specs[i]) for i in range(9)]
        self.item_grid = [
            self.items[0:3],
            self.items[3:6],
            self.items[6:9]
