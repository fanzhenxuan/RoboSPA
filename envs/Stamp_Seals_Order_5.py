...
class Stamp_Seals_Order_5(Base_Task):
    ...
    def load_actors(self):
        self.stage_sum = 5
        self.stage = 0
        self._placed_stages = [False, False, False, False, False]
        ...
        idxs = np.random.choice(len(color_items), size=5, replace=False)
        ...
        self.fourth_color_name, self.fourth_color_value = color_items[idxs[3]]
        self.fifth_color_name, self.fifth_color_value = color_items[idxs[4]]
        ...
        self.target4 = create_visual_box(
            scene=self,
            pose=pad_pose4,
            half_size=half_size,
            color=self.fourth_color_value,
            name="pad4",
        )

        self.target5 = create_visual_box(
            scene=self,
            pose=pad_pose5,
            half_size=half_size,
            color=self.fifth_color_value,
            name="pad5",
        )

        self.pads = [self.target, self.target2, self.target3, self.target4, self.target5]
        ...
        #     or np.linalg.norm(seal_pose.p[:2] - pad_pose4.p[:2]) < 0.12
        #     or np.linalg.norm(seal_pose.p[:2] - pad_pose5.p[:2]) < 0.12
        # ):
        #     seal_pose = rand_pose(
        #         # xlim=[-0.25, 0.10],
        #         xlim=[0.00, 0.25],
        ...
        self.add_prohibit_area(self.target4, padding=0.01)
        self.add_prohibit_area(self.target5, padding=0.01)
    ...
    def update_progress(self):
        ...
        # eps_xy = np.array([0.02, 0.02])
        # xy_ok = np.all(np.abs(seal_pose[:2] - pad_pose[:2]) < eps_xy)
    ...
    def play_once(self):
        ...
        # seal -> pad4
        arm_tag4 = self.pick_and_place_seal(
            self.target4.get_pose(),
            pre_grasp_dis=0.08,
            pre_dis=0.1,
        )
        self.update_progress()
        ...
        # seal -> pad5
        arm_tag5 = self.pick_and_place_seal(
            self.target5.get_pose(),
            pre_grasp_dis=0.08,
            pre_dis=0.1,
        )
        self.update_progress()
        ...
        self.info["info"] = {
            ...
            "{E}": f"{self.fourth_color_name}",
            "{F}": f"{self.fifth_color_name}",
            # "{a}": arm_tag1,
            # "{b}": arm_tag2,
            # "{c}": arm_tag3,
            # "{d}": arm_tag4,
