...
class Pick_Cups_Canonical_5(Base_Task):
    ...
    def load_actors(self):
        ...
        # 由于4类物体尺寸差异较大，这里统一采用更松散、更稳健的3×4排布参数
        center_x = np.random.uniform(-0.03, -0.02)
        center_y = np.random.uniform(-0.11, -0.09)
        col_gap = 0.152
        row_gap = 0.120

        x1 = center_x - 1.5 * col_gap
        x2 = center_x - 0.5 * col_gap
        x3 = center_x + 0.5 * col_gap
        x4 = center_x + 1.5 * col_gap
        ...
        x_jitter = min(0.0015, max(0.004, 0.18 * obj_width))
        x_jitter = min(x_jitter, 0.18 * min_col_gap)

        y_jitter = min(0.0015, max(0.002, 0.10 * obj_depth))
        ...
        pose1 = fixed_item_pose(x1, y1)
        pose2 = fixed_item_pose(x2, y1)
        pose3 = fixed_item_pose(x3, y1)
        pose4 = fixed_item_pose(x4, y1)

        pose5 = fixed_item_pose(x1, y2)
        pose6 = fixed_item_pose(x2, y2)
        pose7 = fixed_item_pose(x3, y2)
        pose8 = fixed_item_pose(x4, y2)

        pose9 = fixed_item_pose(x1, y3)
        pose10 = fixed_item_pose(x2, y3)
        pose11 = fixed_item_pose(x3, y3)
        pose12 = fixed_item_pose(x4, y3)
        ...
        self.item1 = create_item(pose1)
        self.item2 = create_item(pose2)
        self.item3 = create_item(pose3)
        self.item4 = create_item(pose4)
        self.item5 = create_item(pose5)
        self.item6 = create_item(pose6)
        self.item7 = create_item(pose7)
        self.item8 = create_item(pose8)
        self.item9 = create_item(pose9)
        self.item10 = create_item(pose10)
        self.item11 = create_item(pose11)
        self.item12 = create_item(pose12)

        self.items = [
            self.item1, self.item2, self.item3, self.item4,
            self.item5, self.item6, self.item7, self.item8,
            self.item9, self.item10, self.item11, self.item12,
        ]
