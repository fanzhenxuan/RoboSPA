...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        self.seed_value = kwags.get("seed", 0)
        ...
        self.scene_config = kwags.get("scene_config", None)
        ...
        if self.eval_mode:
            with open(os.path.join(CONFIGS_PATH, "_eval_step_limit.yml"), "r") as f:
                try:
                    ...
                    # *******************************************************
                    policy_name = kwags.get("policy_name", "").lower()
                    if policy_name == "xvla":
                        self.step_lim *= 3
                except:
    ...
    def choose_grasp_pose(
        ...
        """
        if not self.plan_success:
            #print("规划失败，choose_grasp_pose直接返回空")
            return
    ...
    def get_place_pose(
        ...
        if pre_dis_axis == "grasp":
            ...
            #print("to_transformation_matrix_1379:",transforms._toPose(target_pose))
