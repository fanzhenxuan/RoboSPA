...
class Move_Blocks_Apart_5(Base_Task):
    ...
    def load_actors(self):
        ...
        self.block2_color_name = color_name_a
        self.block3_color_name = color_name_b
        self.block4_color_name = color_name_b
        self.block5_color_name = color_name_b
        ...
        colors = [color_a, color_a, color_b, color_b, color_b]
        ...
        self.block4 = create_box(scene=self, pose=poses[3], half_size=half_size, color=colors[3], name="box")
        self.block5 = create_box(scene=self, pose=poses[4], half_size=half_size, color=colors[4], name="box")
    ...
    def play_once(self):
        ...
        first_two = [self.block1, self.block2]
        first_two_sorted = sorted(first_two, key=lambda b: b.get_pose().p[1], reverse=True)

        self.pick_and_place_block(
            actor=first_two_sorted[0],
            target_xy=[0.2, 0.05],
            is_last=False,
        )

        self.pick_and_place_block(
            actor=first_two_sorted[1],
            target_xy=[0.2, -0.08],
            is_last=False,
        )

        last_three = [self.block3, self.block4, self.block5]
        last_three_sorted = sorted(last_three, key=lambda b: b.get_pose().p[1], reverse=True)

        self.pick_and_place_block(
            actor=last_three_sorted[0],
            target_xy=[-0.2, 0.05],
            is_last=False,
        )

        self.pick_and_place_block(
            actor=last_three_sorted[1],
            target_xy=[-0.2, -0.08],
            is_last=False,
        )

        self.pick_and_place_block(
            actor=last_three_sorted[2],
            target_xy=[-0.2, -0.20],
            is_last=True,
        )
        ...
        self.info["info"] = {
            ...
            # "{D}": f"{self.block4_color_name} block",
            # "{E}": f"{self.block5_color_name} block",
            "{A}": self.block1_color_name,
            "{B}": self.block3_color_name,
        }
    ...
    def update_progress(self):
        ...
        self.task_success[3] = 1 if self.block4.get_pose().p[0] <= -0.1 else 0
        self.task_success[4] = 1 if self.block5.get_pose().p[0] <= -0.1 else 0
