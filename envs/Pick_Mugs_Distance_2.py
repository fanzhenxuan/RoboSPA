...
class Pick_Mugs_Distance_2(Base_Task):
    """
    Mug距离推理pick任务
    设计原则：
    ...
    2. 仅修改上桌mug的选择逻辑：
       - 先随机选择一个参照颜色
       - 该颜色只放置1个对应base
       - 桌面上其余3个mug从其他颜色对应的base里随机挑选
    3. 参照mug固定为这个唯一颜色的mug，再在其余mug中随机选择“第x远”的mug作为目标
    ...
    # 按mug数量做轻微稳健调整
    MIN_DIST_SQ = 0.025
    CLEAR_DIST_GAP = 0.085
    PROHIBIT_PADDING = 0.078
    ...
    def _select_objects_for_episode(self):
        ...
        for color_text in all_color_texts:
            ...
            if len(other_base_ids) >= self.N_OBJ - 1:
                valid_reference_colors.append(color_text)

        assert len(valid_reference_colors) > 0, "没有可用于当前mug数量的参照颜色"
        ...
        chosen_other_base_ids = list(
            np.random.choice(other_base_ids, size=self.N_OBJ - 1, replace=False)
        )

        selected_pairs = [(reference_base_id, reference_color_text)]
        for base_id in chosen_other_base_ids:
            selected_pairs.append((int(base_id), self.BASE_ID_TO_COLOR[int(base_id)]))
