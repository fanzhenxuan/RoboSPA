...
class Hit_Blocks_Hammer_Order_2(Base_Task):
    ...
    def load_actors(self):
        ...
        # --- 根据第一个方块的 x，确定第二个方块的 x 约束（与你 3/4 方块版本一致） ---
        if poses[0].p[0] < 0:
        ...
        # --- 生成第二个方块（带 x 约束，其它不变） ---
        p1 = sample_block_pose(xlim=constrained_xlim)
        
        # while not valid_pose(p1, poses, min_dist=0.12):
        #     p1 = sample_block_pose(xlim=constrained_xlim)
        
        max_trials = 100
        trials = 0
        
        while not valid_pose(p1, poses, min_dist=0.12) and trials < max_trials:
            p1 = sample_block_pose(xlim=constrained_xlim)
            trials += 1
        
        if not valid_pose(p1, poses, min_dist=0.12):
            raise RuntimeError("Failed to sample a valid p1 within 100 tries.")
        ...
        self._hit_blocks = [False, False]

    # def update_progress(self):
    #     idx = min(self.stage, self.stage_sum - 1)  # 0->box1, 1->box2
    ...
    def play_once(self):
        ...
        self.info["info"] = {"{A}": "020_hammer/base0", # "{B}": str(self.stage_sum), 
        "{a}": str(arm_tag)}
        return self.info
