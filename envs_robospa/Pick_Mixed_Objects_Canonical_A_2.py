...
class Pick_Mixed_Objects_Canonical_A_2(Base_Task):
 ...
 {'modelname': '100_seal', 'skip_model_ids': []}]
    ...
    def _sampled_specs(self):
        specs = []
        for spec in self.OBJECT_SPECS[:6]:
            spec = dict(spec)
    ...
    def load_actors(self):
        ...
        center_x = np.random.uniform(-0.10, 0.06)
        ...
        self.items = [create_item(poses[i], self.object_specs[i]) for i in range(6)]
        self.item_grid = [
            self.items[0:3],
            self.items[3:6]
        ]
