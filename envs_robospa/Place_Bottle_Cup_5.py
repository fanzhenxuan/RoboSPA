...
class Place_Bottle_Cup_5(Base_Task):
    ...
    def load_actors(self):
        self.stage_sum = 5
        self.stage_num = 0
        self.task_success = [0, 0, 0, 0, 0]   # 新增：记录五个子任务是否完成
        ...
        self.cup2, self.coaster2 = create_one_cup_coaster_pair(tag=1)

        self.cups = [self.cup1, self.cup2]
        self.coasters = [self.coaster1, self.coaster2]
    ...
    def play_once(self):
        self.stage_num = 0
        self.task_success = [0, 0, 0, 0, 0]   # 每次执行前重置
        ...
        arm_info_5 = self.place_empty_cup_back(self.cup2, self.coaster2, last_arm=arm_info_4)
        self.update_progress()
    ...
    def place_empty_cup_back(self, cup, coaster, last_arm=None):
        ...
        if last_arm is not None and str(last_arm) != str(arm_tag):
            # last_arm = str(last_arm)
            # if last_arm != str(arm_tag):
            self.move(self.close_gripper(arm_tag, pos=0.6),
            self.back_to_origin(last_arm))
        else:
            self.move(self.close_gripper(arm_tag, pos=0.6))
    ...
    def update_progress(self):
        ...
        # cup 部分仍然允许两个杯子和两个杯垫交换匹配
        cup1_ok = self.cup_on_coaster(self.cup1, self.coaster1) or self.cup_on_coaster(self.cup1, self.coaster2)
        cup2_ok = self.cup_on_coaster(self.cup2, self.coaster1) or self.cup_on_coaster(self.cup2, self.coaster2)

        if cup1_ok:
            self.task_success[3] = 1

        if cup2_ok:
            self.task_success[4] = 1
