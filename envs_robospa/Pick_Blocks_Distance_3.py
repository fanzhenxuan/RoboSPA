...
class Pick_Blocks_Distance_3(Base_Task):
    """
    任务说明：
    - 桌面上有8个不同颜色的block
    - 不再按颜色直接pick，而是按“相对参照物的距离排序”进行pick
    - 每轮随机选择一个参照block，再在其余7个block中随机选择“第k远”的block作为目标
    - 距离排序按“从远到近”定义，k的范围为1st ~ 7th
    ...
    def load_actors(self):
        ...
        size = np.random.uniform(0.015, 0.017)
        ...
        for block in self.blocks.values():
            self.add_prohibit_area(block, padding=0.036)
