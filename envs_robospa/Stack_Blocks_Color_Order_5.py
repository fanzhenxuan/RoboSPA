...
class Stack_Blocks_Color_Order_5(Base_Task):
    ...
    def load_actors(self):
            ...
            for i in range(self.num_blocks):
                ...
                while (
                    (
                        # abs(block_pose.p[0]) < 0.05
                        # or 
                        # np.sum((block_pose.p[:2] - np.array([0, -0.1])) ** 2) < 0.0225
                        # or not 
        ...
        # 按堆叠顺序创建 block：block1 是最底下，blockN 是最上面
        self.blocks = []
        for i in range(self.num_blocks):
            block = create_block(
                block_pose_lst[i],
    ...
    def play_once(self):
        ...
        arm_tags = []
        for block in self.blocks:
            arm_tag = self.pick_and_place_block(block)
    ...
    def update_progress(self):
        ...
        rots = [block.get_pose().to_transformation_matrix()[:3, :3] for block in self.blocks]
        ...
        for i in range(1, self.num_blocks):
            ...
            is_aligned = (
                abs(poses[i][2] - expected_z) <= eps_z
            )
