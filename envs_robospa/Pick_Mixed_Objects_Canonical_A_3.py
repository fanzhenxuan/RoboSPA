...
class Pick_Mixed_Objects_Canonical_A_3(Base_Task):
    ...
    def _sampled_specs(self):
        specs = []
        for spec in self.OBJECT_SPECS[:8]:
            spec = dict(spec)
    ...
    def load_actors(self):
        ...
        center_x = np.random.uniform(-0.05, -0.02)
        center_y = np.random.uniform(-0.12, -0.1)
        col_gap = 0.156
        row_gap = 0.125
        ...
        xs = [x1, x2, x3, x4]
        y_row1 = center_y - row_gap / 2
        y_row2 = center_y + row_gap / 2
        y_values = [y_row1, y_row2]
        ...
        self.items = [create_item(poses[i], self.object_specs[i]) for i in range(8)]
        self.item_grid = [
            self.items[0:4],
            self.items[4:8]
        ]
