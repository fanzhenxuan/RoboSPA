...
class Place_Burger_Fries_Click_Bell_5(Base_Task):
    ...
    def _fries_back_in_place(self):
        target_pose = getattr(self, "fries_home_pose", self.fries_init_pose)
        current_pose = self.frenchfries.get_pose()
        xy_dis = np.linalg.norm(current_pose.p[0:2] - target_pose.p[0:2])
        z_dis = abs(current_pose.p[2] - target_pose.p[2])
        not_on_tray = not self.check_actors_contact(self.frenchfries.get_name(), self.tray.get_name())
        return xy_dis < 0.04 and z_dis < 0.005 and not_on_tray

    def _hamburg_released(self):
        return self.is_left_gripper_open() and len(self.get_gripper_actor_contact_position("006_hamburg")) == 0

    def _fries_released(self):
        return self.is_right_gripper_open() and len(self.get_gripper_actor_contact_position("005_french-fries")) == 0
    ...
    def update_progress(self):
        ...
        fries_returned = int(
            self.fries_was_placed and self._fries_back_in_place() and self._fries_released()
        )
        ...
        self.task_success[4] = fries_returned
    ...
    def _do_return_fries(self, retreat_arm_tag=None):
        arm_tag_right = ArmTag("right")
        ...
        self.move(
            self.place_actor(
                self.frenchfries,
                arm_tag=arm_tag_right,
                target_pose=self.fries_home_fp_pose,
                functional_point_id=0,
                constrain="free",
                pre_dis=0.1,
                pre_dis_axis="fp",
    ...
    def play_once(self):
        ...
        # 1. place hamburg
        # print("[burger_fries_return_play] stage=place_burger", flush=True)
        self._do_place_hamburg()
        self.update_progress()

        # 2. place fries
        # print("[burger_fries_return_play] stage=place_fries", flush=True)
        ...
        # 3. click bell once
        # print("[burger_fries_return_play] stage=click_once", flush=True)
        ...
        # 4. return hamburg and fries to original positions
        # print("[burger_fries_return_play] stage=return_burger", flush=True)
        self._do_return_hamburg(retreat_arm_tag=prev_arm_tag if prev_arm_tag != arm_tag_left else None)
        prev_arm_tag = arm_tag_left
        # print("[burger_fries_return_play] stage=return_fries", flush=True)
        self._do_return_fries(retreat_arm_tag=prev_arm_tag if prev_arm_tag != arm_tag_right else None)
        self.update_progress()
    ...
    def check_success(self):
        self.update_progress()
        success = self.task_success == [1, 1, 1, 1, 1]
        # if not success:
        #     print(
        #         "[burger_fries_return_check]",
        ...
        #         f"fries_returned={self.task_success[4]}",
        ...
        #         f"fries_xy={np.linalg.norm(self.frenchfries.get_pose().p[0:2] - self.fries_home_pose.p[0:2]):.4f}",
        #         f"fries_z={abs(self.frenchfries.get_pose().p[2] - self.fries_home_pose.p[2]):.4f}",
        #         flush=True,
        #     )
        return success
