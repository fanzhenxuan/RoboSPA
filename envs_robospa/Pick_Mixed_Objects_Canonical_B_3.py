...
class Pick_Mixed_Objects_Canonical_B_3(Base_Task):
    ...
    def load_actors(self):
            ...
            center_y = np.random.uniform(-0.12, -0.05)
            col_gap = 0.156
            row_gap = 0.135
            ...
            # 修改点 2：在生成位姿列表时，检查当前位置对应的物体是否为薯条
            poses = []
            idx = 0
            for y in y_values:
                for x in xs:
                    # 获取当前索引对应的物体规格
                    spec = self.object_specs[idx]
                    
                    # 薯条平放用 [1, 0, 0, 0]，其他物体立起用 [0.5, 0.5, 0.5, 0.5]
                    if spec["modelname"] == "005_french-fries":
                        target_qpos = [1, 0, 0, 0]
                    else:
                        target_qpos = [0.5, 0.5, 0.5, 0.5]
                    
                    poses.append(fixed_item_pose(x, y, qpos=target_qpos))
                    idx += 1
